"""Capture screenshot-review AFTER states for 1.3.1-dev final polish."""
...
def main() -> int:
    out = ROOT / "qa_screenshots" / "ui_polish_1_3_1"
    ...
    for w, h in sizes:
        ...
        app.processEvents()
        # History collapsed (default)
        if page._history_expanded:
            page._toggle_history()
        app.processEvents()
        win.grab().save(str(out / f"video_top_{w}x{h}.png"))
        win.grab().save(str(out / f"video_history_collapsed_{w}x{h}.png"))

        # Expanded history
        if not page._history_expanded:
            page._toggle_history()
        app.processEvents()
        win.grab().save(str(out / f"video_history_expanded_{w}x{h}.png"))
        if page._history_expanded:
            page._toggle_history()
    ...
    app.processEvents()
    win.grab().save(str(out / "video_audio_language.png"))
    win.grab().save(str(out / "video_template_branding_composition.png"))
    win.grab().save(str(out / "video_media.png"))
    win.grab().save(str(out / "video_output.png"))

    win.close()
    (out / "capture_ok.txt").write_text("final polish screenshots ok\n", encoding="utf-8")
    print("WROTE", out)
