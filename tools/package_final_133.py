"""Package BMT Voice Studio 1.3.3 FINAL. Never overwrite 1.3.2 / 1.3.1 / 1.3.0 / 1.2 FINAL zips."""
...
DIST_NAME = "BMTVoiceStudio-1.3.3"
PORTABLE_NAME = "BMTVoiceStudio-1.3.3-Windows-x64-Portable"
...
STABLE_132_ZIP = "BMTVoiceStudio-1.3.2-Windows-x64-Portable.zip"
STABLE_132_SHA256 = "02f1f746f6d94c871a6e79115c484bdd49a7afc8266af7d0f27ee662598d5692"

README = """BMT Voice Studio 1.3.3
Windows 10/11 x64
...
- SAR 1:1 / yuv420p normalization for reliable WhatsApp renders
...
def _verify_stable(when: str) -> None:
    checks = (
        ...
        (ROOT / "release" / STABLE_132_ZIP, STABLE_132_SHA256, "1.3.2"),
    )
...
def main() -> int:
    ...
    if version != "1.3.3":
        raise SystemExit(f"STOP: VERSION must be 1.3.3 for FINAL package, got {version!r}")
    ...
    for extra in ("BMTVoiceStudio-1.3.3-dev",):
    ...
    spec = ROOT / "BMTVoiceStudio-1.3.3.spec"
    ...
    if 'name="BMTVoiceStudio-1.3.3"' not in spec_text and "BMTVoiceStudio-1.3.3" not in spec_text:
        raise SystemExit("STOP: FINAL spec must collect as BMTVoiceStudio-1.3.3")
    ...
    test_result = (ROOT / "qa_outputs" / "sar_fix_133" / "pytest_summary.txt").read_text(
        encoding="utf-8"
    ).strip() if (ROOT / "qa_outputs" / "sar_fix_133" / "pytest_summary.txt").is_file() else "see session log"
    ...
    manifest = {
        ...
        "promoted_from": "1.3.2 + SAR/yuv420p normalization fix",
        "stable_1_3_2_sha256": STABLE_132_SHA256,
    ...
    print("NOTE: 1.3.2 FINAL zip was not modified")
    return 0
